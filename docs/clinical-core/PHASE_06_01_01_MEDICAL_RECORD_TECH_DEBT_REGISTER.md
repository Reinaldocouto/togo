...
| MR-DEBT-012 | MedicalRecordListItemResponse ainda não usado | API/Privacidade futura | P3 | Resolvido tecnicamente por decisão formal de contrato reservado para futura listagem segura | Não | Não | Documento da Fase 6.6.4 registra inspeção de uso, ausência de endpoint de listagem, análise das opções e decisão de manter MedicalRecordListItemResponse como contrato reservado, sem implementação prematura. | Risco original mitigado por decisão explícita; listagem futura ainda exige autorização, paginação, minimização e validação de privacidade. | Contrato mantido sem uso funcional e sem endpoint público, com critérios mínimos para futura listagem segura. | Tratado na Fase 6.6.4; evidências finais na Fase 6.6.5. |
